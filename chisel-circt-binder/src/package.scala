...
private[chisel3] object converter {
  ...
  class ConverterContext {
    ...
    private[converter] def visitAttach(symbols: Seq[String]): Unit = {
      val stmt = _createStmt(0 /* FIRRTL_STATEMENT_KIND_ATTACH */, u => {
        val operandsSeg = MemorySegment.allocateNative(FirrtlStatementAttachOperand.sizeof() * symbols.length, memorySession)
        symbols.zipWithIndex.foreach { case(s, i) => {
          val operand = operandsSeg.asSlice(FirrtlStatementAttachOperand.sizeof() * i, FirrtlStatementAttachOperand.sizeof())
          FirrtlStatementAttachOperand.symbol$slice(operand).copyFrom(createMlirStr(s))
        }}
        FirrtlStatementAttach.operands$set(u, operandsSeg.address())
        FirrtlStatementAttach.count$set(u, symbols.length)
      })
